...
# Modified for the mpipi dataset run: graph-topology targets
# (global_efficiency, fragmentation_index, avg_clustering, transitivity,
# degree_assortativity) are not present in mpipi_results_raw.csv and are
# not computable from it, so KESTREL's graph head has been removed and
# it now only predicts the 5 geometric targets.
ALL_TARGETS = GEO_TARGETS       # 5 targets (graph head removed)

# mpipi_results_raw.csv uses different target column names than merged.csv —
# rename to the GEO_TARGETS names so nothing downstream needs to change.
# NOTE: "rg" is mapped to the fit-derived rg_nm_mpipi (not the directly
# measured rg_mean_nm), for consistency with ree (which only has the
# fit-derived form). Swap this if your professor wants the measured Rg instead.
MPIPI_TARGET_RENAME = {
    "rg_nm_mpipi":  "rg",
    "ree_nm_mpipi": "ree",
    "nu_mpipi":     "nu",
...
def full_eval(model, test_loader, ood_loader,
              out_dir, name, device, stats,
              test_df, ood_df, is_kestrel=True):
    """
    stats             : {"mean": Series, "std": Series} over ALL_TARGETS,
                         from the training split -- used to denormalize
                         predictions/targets back to raw units.
    test_df, ood_df   : the split DataFrames used to build test_loader /
                         ood_loader (shuffle=False), so row order lines
                         up with the predictions returned by evaluate().
    """
    print(f"\n{'='*55}\nEvaluation: {name}\n{'='*55}")

    mean_t = torch.tensor(stats["mean"].values, dtype=torch.float32)
    std_t  = torch.tensor(stats["std"].values,  dtype=torch.float32)

    for split, loader, split_df in [("test", test_loader, test_df),
                                     ("ood",  ood_loader,  ood_df)]:
        ...
        # ── per-sequence predictions (raw units, not condensed to R2) ──
        p_raw = p * std_t + mean_t
        t_raw = t * std_t + mean_t

        seq_df = split_df.reset_index(drop=True)
        pred_df = pd.DataFrame({
            "sequence": seq_df["sequence"].values,
            "kingdom":  seq_df["kingdom"].values,
        })
        for i, tgt in enumerate(ALL_TARGETS):
            pred_df[tgt]            = t_raw[:, i].numpy()
            pred_df[f"{tgt}_pred"]  = p_raw[:, i].numpy()
        pred_df.to_csv(
            os.path.join(out_dir, f"{name}_{split}_preds.csv"),
            index=False)
...
def run(data_csv, out_dir="kestrel_output/", max_len=256,
    ...
    df = df.rename(columns={k: v for k, v in MPIPI_TARGET_RENAME.items()
                             if k in df.columns})
    df = df.dropna(subset=["sequence"] + GEO_TARGETS)
    ...
    full_eval(kestrel, test_dl, ood_dl,
              out_dir, "kestrel", device, stats,
              splits["test"], splits["ood"])
    ...
    if all(c in df.columns for c in PHYSCHEM_COLS):
        ...
        full_eval(pc_model, test_dl, ood_dl,
                  out_dir, "physchem_mlp", device, stats,
                  splits["test"], splits["ood"],
                  is_kestrel=False)
