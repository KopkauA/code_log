...
# All targets — A0 is present in BENDER but absent from Human-IDRome CSV
ALL_TARGETS    = ["rg", "ree", "nu", "delta", "a0"]
IDROME_TARGETS = ["rg", "ree", "nu", "delta"]   # A0 absent from idrome release
...
class GeoHead(nn.Module):
    ...
    Output: len(BENDER_TARGETS) geometric targets (rg, ree, nu, delta, a0)
    Trained on BENDER training split including A0.
    """
    def __init__(self, hidden_dim, n_out=len(BENDER_TARGETS),
...
# rename to the BENDER_TARGETS names so nothing downstream needs to change.
...
def evaluate_on_df(model, embeddings, df, t_mean, t_std,
    ...
    # evaluate only on targets the model was trained on
...
def run(bender_csv, out_dir, model_keys, seed=42, device_str=None):
    ...
    bender_ood     = bender_df[bender_df["kingdom"] == "Viruses"].copy()
    bender_non_ood = bender_df[bender_df["kingdom"] != "Viruses"].copy()
    ...
    for model_key in model_keys:
        ...
        results["bender_test"], preds_by_split["bender_test"] = evaluate_on_df(
            model, bender_test_emb, bender_test,
            t_mean, t_std, f"{name} -- BENDER TEST", device)

        results["ood_viruses"], preds_by_split["ood_viruses"] = evaluate_on_df(
            model, bender_ood_emb, bender_ood,
            t_mean, t_std, f"{name} -- OOD Viruses", device)

        all_results[name] = results

        # ── save per-sequence predictions (raw, not condensed to R2) ──
        split_dfs = {"bender_test": bender_test, "ood_viruses": bender_ood}
        for split, split_df in split_dfs.items():
            preds = preds_by_split[split]
            available = [t for t in BENDER_TARGETS if t in split_df.columns]
            pred_df = split_df[["sequence"] + available].reset_index(drop=True).copy()
            for i, tgt in enumerate(available):
                pred_df[f"{tgt}_pred"] = preds[:, i].numpy()
            pred_df.to_csv(
                os.path.join(out_dir, f"{name}_{split}_preds.csv"), index=False)

        # save per-model R2 results (kept alongside, not instead of, per-sequence output)
        rows = []
    ...
    print(f"{'Model':25s} {'BENDER test':>12} {'OOD Viruses':>12}")
    print("-" * 55)
    for name, results in all_results.items():
        print(f"{name:25s} "
              f"{results['bender_test']['nu']:>12.4f} "
              f"{results['ood_viruses']['nu']:>12.4f}")
