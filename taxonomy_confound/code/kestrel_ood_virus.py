...
SATURATION_N       = [500, 1000, 2000, 5000]   # + 10 272 already done
SATURATION_SEEDS   = [42, 67, 93]
SATURATION_TARGETS = ["a0"]                    # A₀ only


def run_saturation(data_csv, out_dir="saturation_output/",
                   max_len=256, n_list=None, seeds=None,
                   epochs=200, patience=25, lr=5e-4):
    """
    Sample n sequences from the non-viral training pool, train KESTREL from
    scratch, evaluate OOD on viral sequences. Repeat for each (n, seed) pair.

    OOD viral set is always the full set of viral sequences in data_csv — it
    never changes across (n, seed) pairs so comparisons are apples-to-apples.

    Results are written to  <out_dir>/saturation_results.csv  with columns:
        n_train, seed, target, r2_ood
    and a wide version:
        <out_dir>/saturation_results_wide.csv  (one row per n_train × seed,
        columns nu_r2, rg_r2, a0_r2)
    """
    n_list = n_list or SATURATION_N
    seeds  = seeds  or SATURATION_SEEDS

    os.makedirs(out_dir, exist_ok=True)
    device = get_device()

    # ── load & separate fixed OOD viral set ──────────────────────────────────
    ...
    print(f"Non-viral pool: {len(pool_df)} | OOD viral (fixed): {len(ood_df)}")

    records = []

    for n in n_list:
        if n > len(pool_df):
            print(f"n={n} exceeds pool size ({len(pool_df)}), skipping.")
            continue

        for seed in seeds:
            run_tag = f"n{n}_seed{seed}"
            print(f"\n{'='*60}")
            print(f"SATURATION RUN: {run_tag}")
            print(f"{'='*60}")
            torch.manual_seed(seed)
            np.random.seed(seed)

            # sample n sequences from non-viral pool
            sampled = pool_df.sample(n=n, random_state=seed).reset_index(drop=True)

            # 90 / 10 train / val split (no in-distribution test needed)
            rng       = np.random.default_rng(seed)
            idx       = rng.permutation(n)
            n_train   = int(0.90 * n)
            train_df  = sampled.iloc[idx[:n_train]].reset_index(drop=True)
            val_df    = sampled.iloc[idx[n_train:]].reset_index(drop=True)
            ...
            # kingdom vocab from training split
            kingdom_vocab = build_kingdom_vocab(train_df)
            n_kingdoms    = len(kingdom_vocab)
            print(f"  n_train={len(train_df)}  n_val={len(val_df)}  "
                  f"n_ood={len(ood_df)}  kingdoms={n_kingdoms}")

            # adaptive batch size — avoids single-batch val sets
            bs = min(128, max(16, n_train // 8))

            kw = dict(num_workers=0, pin_memory=False)
            def make_ds(split):
                return IDPDataset(split, max_len, stats, kingdom_vocab)

            train_dl = DataLoader(make_ds(train_df), batch_size=bs,
                                  shuffle=True,  **kw)
            val_dl   = DataLoader(make_ds(val_df),   batch_size=bs,
                                  shuffle=False, **kw)
            ood_dl   = DataLoader(make_ds(ood_df),   batch_size=bs,
                                  shuffle=False, **kw)
            ...
            # evaluate on fixed OOD viral set
            _, ood_preds, ood_tgts, _ = evaluate(model, ood_dl, device)
            r2 = r2_scores(ood_preds, ood_tgts)

            row = {"n_train": n, "seed": seed}
            for t in SATURATION_TARGETS:
                row[f"{t}_r2"] = r2.get(t, float("nan"))
            records.append(row)

            print(f"\n  OOD R² — a0={row['a0_r2']:.4f}")
    ...
    wide.to_csv(os.path.join(out_dir, "saturation_results_wide.csv"), index=False)
    ...
    long.to_csv(os.path.join(out_dir, "saturation_results.csv"), index=False)

    # ── summary table ─────────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print("SATURATION SUMMARY (mean ± std across seeds)")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--mode",       default="train",
                   choices=["train", "saturation"],
                   help="'train' = standard single run; "
                        "'saturation' = sample-size sweep")
    p.add_argument("--sat_n",      nargs="+", type=int,
                   default=SATURATION_N,
                   help="Training set sizes to sweep (saturation mode)")
    p.add_argument("--sat_seeds",  nargs="+", type=int,
                   default=SATURATION_SEEDS,
                   help="Seeds to repeat per n (saturation mode)")
    p.add_argument("--sat_epochs", default=200, type=int,
                   help="Max epochs per saturation run")
    p.add_argument("--sat_patience", default=25, type=int,
                   help="Early-stop patience per saturation run")
    a = p.parse_args()
