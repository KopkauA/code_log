...
```
Note: `submit_saturation.sh` was later extended to also submit the taxonomy
confound control experiment (documented separately in
`kestrel_taxonomy_confound_control.md`) — the saturation sweep below is
unaffected and still submitted the same way.
