...
def make_splits(df, ood_kingdom=OOD_KINGDOM,
    ...
    2. Optionally restrict the remaining pool to a single kingdom
       (e.g. train_kingdom="Bacteria") before splitting
    3. Cluster-aware 80/10/10 split on remainder
    """
    rng     = np.random.default_rng(seed)
    ood_df  = df[df["kingdom"] == ood_kingdom].copy()
    main_df = df[df["kingdom"] != ood_kingdom].copy()

    if train_kingdom is not None:
        n_before_filter = len(main_df)
        main_df = main_df[main_df["kingdom"] == train_kingdom].copy()
        print(f"Restricting train/val/test pool to kingdom="
              f"'{train_kingdom}': {n_before_filter} -> {len(main_df)}")
...
def run(data_csv, out_dir="kestrel_output/", max_len=256,
        ...
        lr=5e-4, seed=42, train_kingdom="Bacteria"):
    ...
    # split -- train/val/test restricted to train_kingdom (e.g. "Bacteria"),
    # OOD holdout remains OOD_KINGDOM ("Viruses") regardless
    splits = make_splits(df, seed=seed, train_kingdom=train_kingdom)
    stats  = {
...
if __name__ == "__main__":
    ...
    p.add_argument("--train_kingdom", default="Bacteria",
                   help="Kingdom to restrict train/val/test to. "
                        "OOD holdout kingdom (Viruses) is unaffected.")
    a = p.parse_args()
    run(a.data, a.out, a.max_len, a.batch_size,
        a.epochs, a.patience, a.lr, a.seed, a.train_kingdom)
